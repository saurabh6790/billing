  ...
  <div class="space-y-8">
    ...
    <section>
      ...
      <table class="w-full text-sm">
        <thead><tr class="border-b border-outline-gray-2 text-left text-ink-gray-5"><th class="py-2 pr-4 font-normal">Team</th><th class="py-2 pr-4 font-normal">Standing</th><th class="py-2 pr-4 text-right font-normal">Outstanding</th><th class="py-2 font-normal">Overdue invoices</th></tr></thead>
        <tbody>
          <tr v-for="d in delinquent.data || []" :key="d.team" class="border-b border-outline-gray-1">
            <td class="py-2 pr-4 font-medium text-ink-gray-8">{{ d.team }}</td>
            <td class="py-2 pr-4"><Badge variant="subtle" :theme="standingTheme(d.standing)" :label="titleCase(d.standing)" /></td>
            <td class="py-2 pr-4 text-right text-ink-gray-8">{{ money(d.outstanding) }}</td>
            <td class="py-2 text-ink-gray-6">{{ (d.invoices || []).map(i => i.name).join(', ') || '—' }}</td>
          </tr>
          <tr v-if="!(delinquent.data || []).length"><td colspan="4" class="py-3 text-ink-gray-5">None.</td></tr>
        </tbody>
      </table>
    </section>
    ...
    <section>
      ...
      <table class="w-full text-sm">
        <thead><tr class="border-b border-outline-gray-2 text-left text-ink-gray-5"><th class="py-2 pr-4 font-normal">Team</th><th class="py-2 pr-4 font-normal">Invoice</th><th class="py-2 pr-4 text-right font-normal">Amount</th><th class="py-2 pr-4 font-normal">Reason</th><th class="py-2 font-normal">When</th></tr></thead>
        <tbody>
          <tr v-for="f in failures.data || []" :key="f.name" class="border-b border-outline-gray-1">
            <td class="py-2 pr-4 font-medium text-ink-gray-8">{{ f.team }}</td>
            <td class="py-2 pr-4 text-ink-gray-7">{{ f.invoice }}</td>
            <td class="py-2 pr-4 text-right text-ink-gray-8">{{ money(f.amount) }}</td>
            <td class="py-2 pr-4 text-ink-red-4">{{ f.failure_reason || f.failure_code || '—' }}</td>
            <td class="py-2 text-ink-gray-6">{{ (f.creation||'').slice(0,10) }}</td>
          </tr>
          <tr v-if="!(failures.data || []).length"><td colspan="5" class="py-3 text-ink-gray-5">None.</td></tr>
        </tbody>
      </table>
    </section>
  </div>
